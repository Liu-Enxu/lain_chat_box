...
void headerTask(void *params) {
  while(1){
    for(int i=0;i<framesNum;i++){
      ...
      img.pushImage(0,0,TFT_WIDTH,TFT_HEIGHT,redlain_Frames[i]);
      img.drawString("Header: "+String(uxTaskGetStackHighWaterMark(header_Task_Handle)),0,0,2);
...
void TFTTask(void *params) {
  while(1){
    ...
    img.drawString("ms/f: "+String(end-start),0,0,2);
