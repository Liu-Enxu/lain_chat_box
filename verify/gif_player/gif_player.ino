...
void readSDAndPush(char str[]){
  ...
  openFile(str);
  // read file
  char buffer[2048]; int bytesRead = 0; 
  char word[4]; 
  int pixelIdx = 0;
  unsigned long end;
  unsigned long start = millis();
  ...
  while (myFile.available()) {
    bytesRead = myFile.readBytes(buffer, sizeof(buffer)); // read characters one by one from Micro SD Card
    for(int i=0;i<bytesRead;i+=4){
      strncpy(word, &buffer[i], 4);
      uint16_t num = strtol(word, NULL, 16);memset(word, 0, sizeof(word));
      frame[pixelIdx] = num;
      pixelIdx = (pixelIdx+1)%(animation_width*animation_height);
      if(!pixelIdx){
