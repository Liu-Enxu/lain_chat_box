...
void receive() {
  ...
  while (myClient.connected()){
    if(myClient.available()){//watchdog_update();
      ...
      if (packetSize < TCP_buf_size) {
        packetBuf[packetSize] = '\0';  // Null-terminate the buffer
        if (strncmp(packetBuf, END_MESSAGE, strlen(END_MESSAGE)) == 0) {
          ...
          break;  // End of file, stop receiving
        } //else if(strncmp(packetBuf, TEXT_MESSAGE, strlen(TEXT_MESSAGE)) == 0){
        //   debugOLED("Receiving text...","",true);
        //   addTextPageOLED(uint8_t(0), char_size[0], "Receiving text...", "");
        //   transcription.clear();
        //   recv_text = true;
        //   continue;
        // } else if(strncmp(packetBuf, TEXT_END_MESSAGE, strlen(TEXT_END_MESSAGE)) == 0){
        //   debugOLED("text received.","",true);
        //   recv_text = false;
        //   continue;
        // } else if(strncmp(packetBuf, AUDIO_MESSAGE, strlen(AUDIO_MESSAGE)) == 0){
        //   debugOLED("Receiving audio...","",true);
        //   createFile(reWavName,&soundFile);
        //   recv_audio = true;
        //   continue;
        // } else if(strncmp(packetBuf, AUDIO_END_MESSAGE, strlen(AUDIO_END_MESSAGE)) == 0){
