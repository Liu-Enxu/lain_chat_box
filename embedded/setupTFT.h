...
void headerTask(void *params) {
  while (1) {
    ...
    for (int i = 0; i < 2 * (gifPara->framesNum); i++) {
      ...
      // img.drawString("Send: " + String(uxTaskGetStackHighWaterMark(sendSound_Task_Handle)), 0, 36, 2);
      // img.drawString("Recv: " + String(uxTaskGetStackHighWaterMark(recv_Task_Handle)), 0, 48, 2);
